...
#include "whole_body_state_rviz_plugin/WholeBodyTrajectoryDisplay.hpp"
...
// #include <rviz_common/properties/status_level.hpp>
#include <cmath>
#include <limits>
...
void linkUpdaterStatusFunction(rviz_common::properties::StatusLevel level, const std::string &link_name, const std::string &text,
...
WholeBodyTrajectoryDisplay::WholeBodyTrajectoryDisplay()
  ...
  // Category Groups
  target_category_ = new Property("Target", QVariant(), "", this);
  com_category_ = new Property("Center of Mass", QVariant(), "", this);
  contact_category_ = new Property("End-Effector", QVariant(), "", this);
...
void WholeBodyTrajectoryDisplay::onInitialize() {
  ...
  robot_.reset(new rviz_default_plugins::robot::Robot(scene_node_, context_, "Robot: " + getName().toStdString(), this));
...
void WholeBodyTrajectoryDisplay::processMessage(whole_body_state_msgs::msg::WholeBodyTrajectory::ConstPtr msg) {
...
void WholeBodyTrajectoryDisplay::pushBackCoMAxes(const Ogre::Vector3 &axes_position,
  ...
  if (sq_distant >= scale * scale * 0.0032) {
    boost::shared_ptr<rviz_rendering::Axes> axes;
...
PLUGINLIB_EXPORT_CLASS(whole_body_state_rviz_plugin::WholeBodyTrajectoryDisplay, rviz_common::Display)
